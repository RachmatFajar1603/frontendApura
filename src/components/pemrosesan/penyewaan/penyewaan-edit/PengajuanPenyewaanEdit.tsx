...
const EditPenyewaanForm: React.FC = () => {
  const router = useRouter();
  const { id } = useParams();

  const [sewa, setSewa] = useState<Penyewaan | null>(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (id) {
        const penyewaanData = await getPenyewaanById(id as string);
        if (penyewaanData) {
          setSewa(penyewaanData);
          setTujuan(penyewaanData.tujuan);
          setTanggalMulai(dayjs(penyewaanData.tanggalMulai));
          setTanggalSelesai(dayjs(penyewaanData.tanggalSelesai));
          setTtdPenyewa(penyewaanData.ttdPenyewa);
          setBuktiPembayaran(penyewaanData.buktiPembayaran);

          if (penyewaanData.ruangUmumId) {
            setSelectedAssetType('ruanganUmum');

            // Initialize selected facilities from fasilitasPeminjaman
            if (penyewaanData.fasilitasPenyewaan && penyewaanData.fasilitasPenyewaan.length > 0) {
              const initialFasilitas = penyewaanData.fasilitasPenyewaan.map((fp: any) => ({
                id: fp.fasilitasId,
                jumlahDisewa: fp.jumlahDisewa,
              }));
              setSelectedFasilitas(initialFasilitas);
              const origQuantities: Record<string, number> = {};
              penyewaanData.fasilitasPenyewaan.forEach((fp: any) => {
                origQuantities[fp.fasilitasId] = fp.jumlahDisewa;
              });
              setOriginalQuantities(origQuantities);
            }
          } else if (penyewaanData.ruangLabId) {
            setSelectedAssetType('ruanganLab');
          } else if (penyewaanData.alatId) {
  ...
  useEffect(() => {
    if (sewa && selectedAssetType) {
      let selectedAsset;
      const assetQuantity = sewa.jumlahYangDisewa;
      switch (selectedAssetType) {
        case 'ruanganUmum':
          selectedAsset = ruanganUmum.find((asset) => asset.id === sewa.ruangUmumId);
          break;
        case 'ruanganLab':
          selectedAsset = ruanganLab.find((asset) => asset.id === sewa.ruangLabId);
          break;
        case 'alat':
          selectedAsset = alat.find((asset) => asset.id === sewa.alatId);
          break;
      }
      if (selectedAsset) {
        setSelectedAssets([{ ...selectedAsset, quantity: assetQuantity || 1, harga: Number(selectedAsset.harga) }]);
      }
    }
  ...
  const handleFacilityQuantityChange = (facilityId: string, value: string) => {
    ...
    setSelectedFasilitas((prev) => prev.map((f) => (f.id === facilityId ? { ...f, jumlahDisewa: quantity } : f)));
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (ttdPenyewa instanceof File) {
        ...
        // If ttdPeminjam is already a URL, use it directly
        ttdPenyewaUrl = ttdPenyewa;
      }
      ...
      if (buktiPembayaran instanceof File) {
        ...
        // If ttdPeminjam is already a URL, use it directly
        buktiPembayaranUrl = buktiPembayaran;
      }

      const updatedPenyewaan: any = {
        tujuan,
        tanggalMulai: tanggalMulai.format('YYYY-MM-DD'),
        tanggalSelesai: tanggalSelesai.format('YYYY-MM-DD'),
        ttdPenyewa: ttdPenyewaUrl,
        buktiPembayaran: buktiPembayaranUrl,
        jumlahYangDisewa: selectedAssets.reduce((total, asset) => total + asset.quantity, 0),
      ...
      switch (selectedAssetType) {
        case 'ruanganUmum':
          updatedPenyewaan.ruangUmumId = selectedAssets[0]?.id;
          break;
        case 'ruanganLab':
          updatedPenyewaan.ruangLabId = selectedAssets[0]?.id;
          break;
        case 'alat':
          updatedPenyewaan.alatId = selectedAssets[0]?.id;
          break;
      }

      await updatePenyewaan(id as string, updatedPenyewaan);

      console.log('penyewaan berhasil diperbarui:', updatedPenyewaan);

      setSuccessMessage('Penyewaan berhasil diperbarui!');
      setSnackbarSeverity('success');
      setOpenSnackbar(true);
      router.push('/penyewaan'); // Redirect to the borrowing list page
    } catch (error) {
      setErrorMessage(
        `Error saat memperbarui penyewaan: ${
          (error as any).response?.data?.message || (error as any).message || 'Terjadi kesalahan'
        }`
      );
      setSnackbarSeverity('error');
      setOpenSnackbar(true);
      console.error('Error saat memperbarui pernyewaan:', error);
    } finally {
          ...
          const isDisabled =
            ...
            (facility.jumlah <= 0 ||
              (facility.statusAset !== 'TERSEDIA' &&
                facility.statusAset !== 'SEDANG_DISEWA' &&
                facility.statusAset !== 'SEDANG_DIPINJAM'));
          const quantity = selectedFacility?.jumlahDisewa || 0;
                ...
                onBlur={() => {
                  if (isChecked) {
                    setFacilityQuantities((prev) => ({
                      ...prev,
                      [facility.id]: selectedFasilitas.find((f) => f.id === facility.id)?.jumlahDisewa || 0,
                    }));
                  }
                }}
              />
  ...
  const renderAssetList = () => {
    ...
    switch (selectedAssetType) {
      case 'ruanganUmum':
        assets = ruanganUmum.map((asset) => ({
          ...asset,
          harga: typeof asset.harga === 'string' ? parseFloat(asset.harga) : asset.harga ?? 0,
        })) as AssetDetails[];
        break;
      case 'ruanganLab':
        assets = ruanganLab.map((asset) => ({
          ...asset,
          harga: typeof asset.harga === 'string' ? parseFloat(asset.harga) : asset.harga ?? 0,
        })) as AssetDetails[];
        break;
                    ...
                    >
                        ...
                        >
                          ...
                          <Box sx={{ textAlign: 'right' }}>
                            <Typography variant="body2">
                              {asset.quantity} x {asset.harga.toLocaleString('id-ID')} / hari
                            </Typography>
                            <Typography variant="body2" color="primary.dark" fontWeight="bold">
                              Total: {(asset.quantity * asset.harga).toLocaleString('id-ID')}
                            </Typography>
                          </Box>
                        </Box>
                      ))}
                      ...
                      >
                        ...
                        <Typography variant="h6" color="primary.main" fontWeight="bold">
                          Rp{' '}
                          {selectedAssets
                            .reduce((total, asset) => total + asset.quantity * asset.harga, 0)
        ...
        >
          ...
          {activeStep === steps.length - 1 ? (
            <Button
              ...
              endIcon={isSubmitting ? <CircularProgress size={24} /> : <Check />}
              sx={{
                flex: { xs: '1', sm: '1 1 auto' },
...
export default EditPenyewaanForm;
