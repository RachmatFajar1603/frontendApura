...
function PerbaikanForm() {
  ...
  const { postPerbaikan, error: postError } = usePerbaikan();
  const { user } = useUsers();
  ...
  const handleSubmit = async () => {
    if (selectedAssets.length === 0 || !deskripsi) {
    ...
    // Construct the dynamic key based on the selected asset type
    const assetIdKey = {
      ruanganUmum: 'ruangUmumId',
      ruanganLab: 'ruangLabId',
      alat: 'alatId',
      fasilitas: 'fasilitasId',
    }[selectedAssetType];

    // Build the POST request body with the appropriate asset ID
    const newPerbaikan = {
      id: '', // Provide a valid id or generate one
      namaPenyetujuPerbaikanId: '', // Provide a valid id
      namaPengajuId: user?.id || '', // Assuming user id is available
      kondisiAset: 'Baik', // Provide a valid condition
      [assetIdKey!]: selectedAssets.length > 0 ? selectedAssets[0].id : undefined,
      deskripsi,
      departemenId: selectedAssets.length > 0 ? selectedAssets[0].departemenId : undefined,
      jumlahYangDiPerbaiki: selectedAssets.length > 0 ? selectedAssets[0].quantity : undefined,
    };

    try {
      const response = await postPerbaikan({
        ...newPerbaikan,
        statusAset: 'Pending', // Provide a valid status
        statusPengajuan: 'Pending', // Provide a valid status
      });
      if (response) {
        setSuccessMessage('Repair request submitted successfully!');
        setSnackbarSeverity('success');
      } else {
        throw new Error('Submission did not return a valid response.');
      }
      router.push('/perbaikan');
    } catch (error) {
      setErrorMessage((error as Error).message || 'An error occurred during submission.');
      setSnackbarSeverity('error');
    } finally {
      setOpenSnackbar(true);
          ...
          >
            <CardContent>
              ...
              >
                ...
                <FormControlLabel value="fasilitas" control={<Radio />} label="Fasilitas" />
              </RadioGroup>
            </CardContent>
          </Card>
        );
